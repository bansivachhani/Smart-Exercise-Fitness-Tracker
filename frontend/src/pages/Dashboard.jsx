...
const recommendVegDiet = (goal) => {
  const vegDietOptions = {
    weight_loss: {
      breakfast: ["Oats with skimmed milk", "Banana", "Almonds (5-6)"],
      lunch: ["Quinoa Salad", "Vegetable Stir-Fry", "Lentil Soup"],
      snacks: ["Fruit Smoothie", "Mixed Nuts", "Green Tea with chickpeas"],
      dinner: ["Brown Rice", "Steamed Vegetables", "Paneer Bhurji (low oil)"],
    },
    weight_gain: {
      breakfast: [
      ...
      snacks: ["Nuts & Seeds", "Protein Shake (Plant-Based)", "Granola Bar"],
      dinner: ["Vegetable Biryani", "Paneer Tikka", "Steamed Veggies"],
    },
    fitness: {
      breakfast: [
      ...
      snacks: ["Boiled Eggs (omit if vegan)", "Fruits", "Trail Mix"],
      dinner: [
        "Lentil Soup",
        "Grilled Vegetables",
  ...
  return vegDietOptions[goal] || vegDietOptions.fitness;
};
...
export default function Dashboard() {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      setUser({ ...user, completedYoga, completedMeditation });
  ...
  const generateRecommendations = () => {
    ...
    const diet = recommendVegDiet(goal);
  ...
  const handleCompleteActivity = async (item) => {
    try {
      const field =
        item.type === "Yoga" ? "completedYoga" : "completedMeditation";
      const today = new Date().toISOString().split("T")[0];

      await axios.post(
        `http://localhost:8000/user/${user.id}/complete-activity`,
        {
          task: item.task,
          type: item.type,
          date: today,
      ...
      const updatedField = [
        ...(user[field] || []),
        { task: item.task, date: today },
      ];
      if (setUser) setUser({ ...user, [field]: updatedField });

      updateTodayAndHistory(
        field === "completedYoga" ? updatedField : user.completedYoga || [],
        field === "completedMeditation"
          ? updatedField
  ...
  const userGoals = [
    {
      id: 1,
      title: "Complete Yoga sessions",
      target: yogaList.length,
      completed: user.completedYoga?.length || 0,
      type: "Yoga",
    },
    {
      id: 2,
      title: "Complete Meditation sessions",
      target: meditationList.length,
      completed: user.completedMeditation?.length || 0,
      type: "Meditation",
    },
  ];
  ...
  const completedActivities =
    (user.completedYoga?.length || 0) + (user.completedMeditation?.length || 0);
  const progressPercentage = totalGoals
    ? Math.round((completedActivities / totalGoals) * 100)
    : 0;
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 mt-16 space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-gray-800 p-6 rounded-3xl shadow-xl border border-gray-700">
            ...
            {user.activity?.length > 0 ? (
              <ActivityChart data={user.activity} dark />
            ) : (
          ...
          <div className="bg-gray-800 p-6 rounded-3xl shadow-xl border border-gray-700 flex flex-col items-center">
            ...
            <div className="w-44 h-44 relative">
              ...
              <span className="absolute inset-0 flex items-center justify-center text-xl font-bold text-gray-100">
                {progressPercentage}%
              </span>
            </div>
            <p className="mt-3 text-gray-400 font-medium">
              of your monthly goal achieved
            </p>
          </div>
        </div>
        ...
        <section className="mb-12">
          ...
          {dietPlan ? (
            ...
            <p className="text-gray-400">No diet plan available for today.</p>
          )}
        </section>
          ...
          <div className="grid gap-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                ...
                >
                  ...
                  {!isCompleted && (
                    <button
                      ...
                      className="mt-3 px-3 py-1 bg-green-500 text-gray-900 rounded-md font-semibold hover:bg-green-600 transition"
                    >
                      Mark as Completed
                    </button>
                  )}
            ...
            {userSchedule.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`bg-gray-800 p-6 rounded-3xl shadow-xl border ${
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => !isCompleted && handleCompleteActivity(item)}
                    className={`mt-3 px-3 py-1 rounded-md font-semibold transition 
          ${
            isCompleted
